...
The cost is that updating is a manual re-copy. That is the right trade for six files.
...
The only edit to upstream source: `#include "Utility/dsp.h"` flattened to
`#include "dsp.h"`, so one include directory suffices.
